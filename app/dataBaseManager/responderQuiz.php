...
if($btnResposta){
    ...
    if($_SESSION['clusterAtual'] == 1){
        $sql = "INSERT INTO respostasQuiz(idQuiz,idUsuario,c1,r1) VALUES ($idQuiz,$idUsuario,$idCluster,$nImage)";
    }
    else{
        $clusterSt = 'c'.$_SESSION['clusterAtual'];
        $respostaSt = 'r'.$_SESSION['clusterAtual'];
        $sql = "UPDATE respostasQuiz SET $clusterSt = $idCluster, $respostaSt = $nImage WHERE idQuiz = $idQuiz AND idUsuario = $idUsuario ";
    }
