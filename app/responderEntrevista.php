    ...
    <div class="container-fluid">
        <form method="POST" action="./dataBaseManager/responderEntrevista.php">
            <?php
                ...
                while($row = mysqli_fetch_assoc($result)) { $cont = 1; $id = $row["id"]; $nome = $row["nome"]; $descricao = $row["descricao"]; 
            ?>
                        ...
                        <?php while($cont <= 20) { $pergunta = "p".$cont; if($row["$pergunta"]!=""){ $resposta = "r".$cont ?>
                            <div class="row">
                                ...
                                <input class="form-control campoDefault" placeholder="Sua resposta aqui" name="<?php echo $resposta ?>" required/>
                            </div>
                        <?php 
                            }
                            $cont++;
                        ...
                        <button class="btEstilo" type="submit" name='btnCadastra' value="<?php echo $id?>" style="padding-left: 10px; padding-right:10px;width:100px;margin-left:2px">Enviar</button>
